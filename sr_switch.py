...
class sr_switch():
    ...
    #def link_aggregation(self, ports):

    def get_buckets(self, parser, next_hop, actions=[]):

        port = self.get_port(next_hop)

        if len(port) > 1:
            LOG.warn("multiple links to the same destination not yet supported. Usining first link only")
            #self.link_aggregation(port)

        else:
            port = port[0]

        actions.append(parser.NXActionRegLoad(ofs_nbits=nicira_ext.ofs_nbits(0, 31), dst="in_port",value=0))

        actions.append(parser.OFPActionOutput(port))


        return parser.OFPBucket(watch_port=port, actions=actions)
    ...
    def handle_fw(self, paths, labels, next_hops, n_labels, n_next_hops, switch):
        for d in paths:
            if len(paths[d]) > 1:
                ...
                buckets = [self.get_buckets(parser, next_hop, actions)]
                n_buckets = buckets[:]
    ...
    def back_up_buckets(self, lb, nexthop, parser, ofp, next_hop):
        ...
        bucket = self.get_buckets(parser, nexthop, actions=acts)
